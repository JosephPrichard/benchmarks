module Puzzle = struct
  ...
  let in_bounds ((row, col) : position) size = row >= 0 && row < size && col >= 0 && col < size

  let ( ++ ) ((row1, col1) : position) ((row2, col2) : position) : position = (row1 + row2, col1 + col2)

  let ( .%() ) board pos =
    if in_bounds pos board.size then
      board.tiles.(index_of_pos pos board.size)
    else
      raise
        (Invalid_argument
          (Printf.sprintf
  ...
  let find_empty board =
    let i = board.tiles 
      ...
      |> fst
    in
    pos_of_index i board.size

  let swap board (posl : position) (posr : position) =
    let try_swap i tile =
      let pos = pos_of_index i board.size in
      if pos = posl then board.%(posr)
      else if pos = posr then board.%(posl)
      else tile
    in
    { tiles = Array.mapi try_swap board.tiles; size = board.size }

  let manhattan_dist (row1, col1) (row2, col2) = abs (row2 - row1) + abs (col2 - col1)

  let heuristic board =
    let foldh (sum, i) tile =
      if tile == 0 then sum, i + 1
      else 
        let dst = manhattan_dist (pos_of_index i board.size) (pos_of_index tile board.size) in
        sum + dst, i + 1
    in
  ...
  let hash_tiles tiles : int64 =
    let hash_tile hash tile i = 
      let tile = Int64.of_int tile in
      let mask = Int64.shift_left tile (i * 4) in
      Int64.logor hash mask
    in
    fst (Array.fold_left 
      (fun (hash, i) tile -> (hash_tile hash tile i), (i + 1)) 
      (0L, 0) 
      tiles)

  let directions = [ ((0, 1), Right); ((1, 0), Down); ((0, -1), Left); ((-1, 0), Up) ]

  let adj_puzzles puzzle =
    let adj_directions pos size = directions
      |> List.map (fun (direction, move) -> pos ++ direction, move)
      |> List.filter (fun (pos, _) -> in_bounds pos size)
    in
    let empty_pos = find_empty puzzle.board in
    let fold_directions acc (next_pos, move) =
      let board = swap puzzle.board empty_pos next_pos in
      let gscore = puzzle.gscore + 1 in
      { parent = Some puzzle
      ; board = board
      ; gscore
      ; fscore = gscore + heuristic board
      ; move
      }
      :: acc
    in
    List.fold_left fold_directions [] (adj_directions empty_pos puzzle.board.size)
...
let rec search frontier visited goal_tiles nodes =
  ...
  | Some (key, puzzle) ->
    ...
    if puzzle.board.tiles = goal_tiles then
      reconstruct_path puzzle [], nodes
    else
      let frontier = add_neighbors (Puzzle.adj_puzzles puzzle) (OrderedPuzzles.remove key frontier) visited in
...
let solve tiles =
  ...
  in
  let frontier = OrderedPuzzles.add (Puzzle.hash_tiles initial.board.tiles) initial OrderedPuzzles.empty in
  let goal = Puzzle.create_goal (Array.length initial.board.tiles) in
